<script lang="ts">
	...
	let allSlices = get(slices);
	let slice = allSlices.find(sl => sl.id === $page.params.id);
	
	let name = $state(slice?.name || '');
	let goal = $state(slice?.goal || '');
	...
	function edit(event) {
		...
		const currentSlices = get(slices);
		slices.set(
